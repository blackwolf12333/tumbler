...
#ifdef DEBUG
  g_print ("Initializing the Tumbler Comicbook Thumbnailer plugin\n");
#endif

  /* register the types provided by this plugin */
  comicbook_thumbnailer_register (plugin);
  comicbook_thumbnailer_provider_register (plugin);

  /* set up the plugin provider type list */
  type_list[0] = TYPE_COMICBOOK_THUMBNAILER_PROVIDER;
}
...
{
#ifdef DEBUG
  g_print ("Shutting down the Tumbler Comicbook Thumbnailer plugin\n");
#endif
}
